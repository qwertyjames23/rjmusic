        ...
        <div>
                    ...
                    {value.map((url) => (
                        <div key={url} className="relative aspect-square rounded-xl overflow-hidden border border-white/10 bg-[#1c222b] group">
                            ...
                            <button
                                ...
                                className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-lg hover:bg-red-600 transition-all opacity-0 group-hover:opacity-100 shadow-lg"
                                title="Remove image"
                            >
            ...
            {value.length === 0 && (
                <p className="text-xs text-gray-500 mt-2 text-center">
                    💡 Tip: Upload at least one product image for better visibility
                </p>
            )}
        </div>
    );
